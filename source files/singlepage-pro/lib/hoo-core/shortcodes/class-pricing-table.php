...
class Hoo_PricingTable {
    ...
    public function __construct() {
		...
		add_filter( 'hoo_attr_pricingtable-shortcode', array( $this, 'attr' ) );
		add_filter( 'hoo_attr_pricingtable-shortcode-column-wrapper', array( $this, 'column_wrapper_attr' ) );	
		add_filter( 'hoo_attr_pricingtable-shortcode-price', array( $this, 'price_attr' ) );
		add_filter( 'hoo_attr_pricingtable-shortcode-row', array( $this, 'row_attr' ) );
		add_filter( 'hoo_attr_pricingtable-shortcode-footer', array( $this, 'footer_attr' ) );
    ...
    function render_parent( $args, $content = '') {
		...
		$html = sprintf( '%s<div %s>%s</div>', $styles, HooCore_Plugin::attributes( 'pricingtable-shortcode' ), do_shortcode($content) );
	...
	function attr() {
        ...
        $attr['class'] = sprintf( 'singlepage-pricing-table pricing-table-%s %s-boxed-pricing row singlepage-columns-%s columns-%s singlepage-clearfix', 
    ...
    function render_child_column( $args, $content = '') {
		...
		$defaults = HooCore_Plugin::set_shortcode_defaults(
			array(
				'class' 	=> 'singlepage-pricingtable-column',
				'id' 		=> '',			
				'standout'	=> 'no',
				'title'		=> '',
			), $args
		...
		$html = sprintf( '<div %s><div %s><div %s><div %s><h3 %s>%s</h3></div>%s', HooCore_Plugin::attributes( 'pricingtable-shortcode-column-wrapper' ), 
						 HooCore_Plugin::attributes( 'panel-container' ), HooCore_Plugin::attributes( 'panel' ), HooCore_Plugin::attributes( 'panel-heading' ), 
						 HooCore_Plugin::attributes( 'title-row' ), $title, do_shortcode( $content ) );
	...
	function column_wrapper_attr() {
		...
		$attr['class'] = 'panel-wrapper singlepage-column column col-lg-' . $columns . ' col-md-' . $columns . ' col-sm-' . $columns;
    ...
    function render_child_price( $args, $content = '') {
		...
		if( isset( $price ) 
			...
			if( $currency_position != 'right' ){
				$pricing = sprintf( '<span %s>%s</span>', HooCore_Plugin::attributes( 'currency' ), $currency );
			}
			
			$pricing .= sprintf( '<span %s>%s</span>', HooCore_Plugin::attributes( 'integer-part' ), $price[0] );			
			
			if( array_key_exists( '1', $price ) ) {
				$pricing .= sprintf( '<sup %s>%s</sup>', HooCore_Plugin::attributes( 'decimal-part' ), $price[1] );
			}
			...
			if( $currency_position == 'right' ){
				...
				$pricing .= sprintf( '<span %s>%s</span>', HooCore_Plugin::attributes( $currency_classes ), $currency );
				
				if( $time ) {
					$pricing .= sprintf( '<span %s>%s</span>', HooCore_Plugin::attributes( $time_classes ), $time );
				}				
			}			
			...
			$html = sprintf( '<div %s><div %s>%s</div></div>%s', HooCore_Plugin::attributes( 'panel-body pricing-row' ), HooCore_Plugin::attributes( 'price ' . $pricing_class ), 
							  $pricing, do_shortcode( $content ) );
			
		} else {
		
			$html = sprintf( '<div %s></div>%s', HooCore_Plugin::attributes( 'panel-body pricing-row' ), do_shortcode( $content ) );
    ...
    function render_child_row( $args, $content = '') {
		...
		if( $this->is_first_row ) {
			$html = sprintf( '<ul %s>', HooCore_Plugin::attributes( 'list-group' ) );
			$this->is_first_row = false;
		}	
		
		$html .= sprintf( '<li %s>%s</li>', HooCore_Plugin::attributes( 'list-group-item normal-row' ), do_shortcode( $content ) );
    ...
    function render_child_footer( $args, $content = '') {
		...
		$html = sprintf( '</ul><div %s>%s</div>', HooCore_Plugin::attributes( 'panel-footer footer-row' ), do_shortcode( $content ) );
