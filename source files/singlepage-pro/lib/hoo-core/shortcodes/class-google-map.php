...
class Hoo_GoogleMap {
	...
	public function __construct() {
		...
		add_filter( 'hoo_attr_google-map-shortcode', array( $this, 'attr' ) );
	...
	function render( $args, $content = '' ) {
		...
		if( $address ) {
			...
			if( $map_style == 'theme' ) {
				...
				$animation = 'yes';
				$infobox = 'custom';
				$infobox_background_color = singlepageCore_Plugin::hex2rgb( $smof_data['primary_color'] );
				...
				$brightness_level = singlepageCore_Plugin::calc_color_brightness( $smof_data['primary_color'] );
			...
			if( $icon == 'theme' && $map_style == 'custom' ) {
				for( $i = 0; $i < $num_of_addresses; $i++ ) {
					$icon_array[$i] = get_template_directory_uri( ).'/lib/singlepage/images/map_marker.png';						
				}
			}			
			...
			$map_id = uniqid( 'singlepage_map_' ); // generate a unique ID for this map
			$this->map_id = $map_id;
			...
			jQuery(document).ready(function($){
				...
				var markers = [];
				var counter = 0;
				function singlepage_run_map_<?php echo $map_id ; ?>() {
					jQuery('#<?php echo $map_id ; ?>').singlepage_maps({
						...
						overlay_color_hsl: <?php echo json_encode( singlepage_rgb2hsl( $overlay_color ) ); ?>,
				...
				google.maps.event.addDomListener(window, 'load', singlepage_run_map_<?php echo $map_id ; ?>);
				});
			</script>
			<?php
			if( $defaults['id'] ) {
				$html = ob_get_clean() . sprintf( '<div id="%s"><div %s></div></div>', $defaults['id'], HooCore_Plugin::attributes( 'google-map-shortcode' ) );
			} else {
				$html = ob_get_clean() . sprintf( '<div %s></div>', HooCore_Plugin::attributes( 'google-map-shortcode' ) );
			}
	...
	function attr() {
		...
		$attr['class'] = 'shortcode-map singlepage-google-map';
		...
		if ( $force_refresh || 
			...
			if ( $response['response']['code'] == 200 ) {
				...
				if ( $data->status === 'OK' ) {
				  	...
				  	return __( 'No location found for the entered address.', 'singlepage' );
				} elseif( $data->status === 'INVALID_REQUEST' ) {
				   	return __( 'Invalid request. Did you enter an address?', 'singlepage' );
				} else {
					return __( 'Something went wrong while retrieving your map, please ensure you have entered the short code correctly.', 'singlepage' );
				}

			} else {
			 	return __( 'Unable to contact Google API service.', 'singlepage' );
			}
