...
/*	$container_width           = of_get_option( 'content_container_width_'.$i, '' );
	$container_left            = of_get_option( 'content_container_left_'.$i, '');
	$container_top             = of_get_option( 'content_container_top_'.$i, '' );
	$container_padding         = of_get_option( 'content_container_padding_'.$i, '' );*/
	...
	if( $section_type == 'slider' && $section_slider > 0 ){
		...
		$output .= '<section class="section '.esc_attr($class).'" style="" id="'.$menu_slug.'">';
   ...
   if(!$.browser.msie){
	...
	if( $section_type == 'content'){
		 if( $section_full_width == 1){
			...
			<div class="section-full-content" style="'.str_replace('"','\'',$content_style).'">'.do_shortcode( $content ).'</div>';
