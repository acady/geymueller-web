define(['controllers/module', 'openseadragon', 'd3'], function (controllers, openseadragon, d3) {
  ...
  controllers.controller('SearchModalController', ['$scope', '$q', '$http', '$timeout', 'hit', 'descriptions', 'ensembles', 'data', 'searchkeyword','hideClose', 'config', 'imageservice', 'dataservice', function ($scope, $q, $http, $timeout, hit, descriptions, ensembles, data, searchkeyword, hideClose, config, imageservice, dataservice) {
    
    $scope.hit = hit;
    $scope.descriptions = descriptions;
    $scope.hideClose = hideClose;
    $scope.searchkeyword = searchkeyword;
    $scope.ensembles = ensembles;
    $scope.data = data;
    
    var params = {
          page: 0,
          hitsPerPage: 999,
          restrictSearchableAttributes: 'archivnr'
      };
    ...
    $timeout(function(){
      ...
      var viewer = OpenSeadragon({
        hash: Date.now(),
        element: angular.element('.hit-image-modal')[0],
        showNavigator: true,
        prefixUrl: "/content/themes/imareal/js/vendor/openseadragon/built-openseadragon/openseadragon/images/",
        tileSources: imageservice.retrieveDziImageUrl(hit),
        tilesUrl: 'http://cf000036.sbg.ac.at/iipsrv/iipsrv.fcgi?Deepzoom=/7019493.tif_files/'
//        tilesUrl: 'http://cf000036.sbg.ac.at/iipsrv/iipsrv.fcgi?Deepzoom=/media/thumbnail/pyramids/7019493.tif_files/'

      });
      


      // da kommt der code hinein
      ...
      var simulation = d3.forceSimulation()
          ...
          var detailImages = [];
          var alternativImages = [];
          _.each(data.dianr, function(dia){
            
            if($scope.hit.dianr[0] === dia.value && dia.noDisplay){
              $scope.hideZoomImage = true;
            }
            
            if(dia.flags.detail){
              detailImages.push({dianr: [dia.value]});
            }
            if(dia.flags.alternativ){
              alternativImages.push({dianr: [dia.value]});
            }
          });
          $scope.detailImages = detailImages;
          $scope.alternativImages = alternativImages;
        ...
        $q.all(graph_parse_entities_promises).then(function(graph_parse_entities_list){
            ...
            var legend = svg.selectAll(".legend")
                ...
                .enter().append("g")
                .attr("class", "legend")
                .attr("transform", function(d, i) { return "translate("+ (width_ext_legend + 200) +"," + ((i * 20) + (height_ext - 100)) + ")"; });
