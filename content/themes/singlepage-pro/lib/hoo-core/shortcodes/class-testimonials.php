...
class Hoo_Testimonials {
	...
	public function __construct() {
		...
		add_filter( 'hoo_attr_testimonials-shortcode', array( $this, 'attr' ) );
		add_filter( 'hoo_attr_testimonials-shortcode-testimonials', array( $this, 'testimonials_attr' ) );
		add_filter( 'hoo_attr_testimonials-shortcode-quote', array( $this, 'quote_attr' ) );		
		add_filter( 'hoo_attr_testimonials-shortcode-blockquote', array( $this, 'blockquote_attr' ) );	
		add_filter( 'hoo_attr_testimonials-shortcode-review', array( $this, 'review_attr' ) );		
		add_filter( 'hoo_attr_testimonials-shortcode-thumbnail', array( $this, 'thumbnail_attr' ) );
		add_filter( 'hoo_attr_testimonials-shortcode-image', array( $this, 'image_attr' ) );
		add_filter( 'hoo_attr_testimonials-shortcode-author', array( $this, 'author_attr' ) );
		add_filter( 'hoo_attr_testimonials-shortcode-pagination', array( $this, 'pagination_attr' ) );
	...
	function render_parent( $args, $content = '') {
		...
		#singlepage-testimonials-%s a{border-color: {$textcolor};}#singlepage-testimonials-%s a:hover, #singlepage-testimonials-%s .activeSlide{background-color: {$textcolor};}
		.singlepage-testimonials.%s.singlepage-testimonials-%s .author:after{border-top-color:{$backgroundcolor} !important;}</style>", 
		...
		if( self::$parent_args['design'] == 'clean' ) {
			$pagination  = sprintf( '<div %s></div>', HooCore_Plugin::attributes( 'testimonials-shortcode-pagination' ) );
		}

		$html = sprintf( '<div %s>%s<div %s>%s</div>%s</div>', HooCore_Plugin::attributes( 'testimonials-shortcode' ), $styles,
						 HooCore_Plugin::attributes( 'testimonials-shortcode-testimonials' ), do_shortcode($content), $pagination );
	...
	function attr() {
		...
		$attr['class'] = sprintf( 'singlepage-testimonials %s singlepage-testimonials-%s', self::$parent_args['design'], $this->testimonials_counter );
	...
	private function render_child_classic( $content ) {
		...
		if( self::$child_args['name'] ) {
			...
			if( self::$child_args['avatar'] == 'image' && 
				...
				$image_id = HooCore_Plugin::get_attachment_id_from_url( self::$child_args['image'] );
				self::$child_args['alt'] = '';
				if( $image_id ) {
					self::$child_args['alt'] = get_post_field( 'post_excerpt', $image_id );
				}

				$pic = sprintf( '<img %s />', HooCore_Plugin::attributes( 'testimonials-shortcode-image' ) );
			}
			...
			if( self::$child_args['avatar'] != 'none' ) {
				$thumbnail = sprintf( '<span %s>%s</span>', HooCore_Plugin::attributes( 'testimonials-shortcode-thumbnail' ), $pic );
			}
			
			$inner_content .= sprintf( '<div %s>%s<span %s>', HooCore_Plugin::attributes( 'testimonials-shortcode-author' ), $thumbnail, HooCore_Plugin::attributes( 'company-name' ) );
		...
		$html = sprintf( '<div %s><blockquote><q %s>%s</q></blockquote>%s</div>', HooCore_Plugin::attributes( 'testimonials-shortcode-review' ), 
						 HooCore_Plugin::attributes( 'testimonials-shortcode-quote' ), do_shortcode( $content ), $inner_content );
	...
	private function render_child_clean( $content ) {
		...
		if( self::$child_args['avatar'] != 'none' ) {
			$thumbnail = sprintf( '<div %s>%s</div>', HooCore_Plugin::attributes( 'testimonials-shortcode-thumbnail' ), $pic );
		}	
	
		$author .= sprintf( '<div %s><span %s>', HooCore_Plugin::attributes( 'testimonials-shortcode-author' ), HooCore_Plugin::attributes( 'company-name' ) );	
		...
		$html = sprintf( '<div %s>%s<blockquote %s><q %s>%s</q></blockquote>%s</div>', HooCore_Plugin::attributes( 'testimonials-shortcode-review' ), $thumbnail,
						 HooCore_Plugin::attributes( 'testimonials-shortcode-blockquote' ), HooCore_Plugin::attributes( 'testimonials-shortcode-quote' ), 
	...
	function pagination_attr() {
		...
		$attr['id'] = sprintf( 'singlepage-testimonials-%s', $this->testimonials_counter );
