define(['controllers', 'directives', 'filters', 'lodash'], function () {
  'use strict';
  var app = angular.module('app', [ 'app.controllers', 'app.directives', 'app.filters', 'chart.js', 'ui.bootstrap', 'ui.router', 'ngSanitize', 'smoothScroll', 'ui-notification', 'rzModule', 'treeControl', 'ngAnimate', 'angular-loading-bar', 'LocalStorageModule']);
        ...
        resolve:{
            hit: ['config', '$http','$location', '$q', function(config, $http, $location, $q){
              ...
              $http.get(config.solrBaseUrl + '/solr/work/select', {
                    params: {
                        'q' : 'archivnr:' + $location.search().archivnr,
                        'wt': 'json'
                      }
            ...
            descriptions: ['hit', 'productservice', function(hit, productservice){
              return productservice.retrieveDescriptions(hit);
            }],
            ensembles: ['hit', 'productservice', function(hit, productservice){
