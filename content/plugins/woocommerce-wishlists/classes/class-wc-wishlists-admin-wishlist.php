...
class WC_Wishlists_Wishlist_Admin {
	...
	function add_metaboxes() {
		...
		add_meta_box( 'wc_wishlists_notifications', __( 'Notifications', 'wc_wishlist' ), array(&$this, 'notifications_metabox'), 'wishlist', 'advanced', 'default' );
	}
	...
	public function sharing_metabox( $post ) {
		...
		<div class="wl-admin-wrapper">
			...
			<table class="wl-rad-table">
				<tr>
					<td><input type="radio" name="wishlist_sharing" id="rad_pub" value="Public" <?php checked( 'Public', $sharing ); ?>><label for="rad_pub" ><?php _e( 'Public', 'wc_wishlist' ); ?> <span class="wl-small">- <?php _e( 'Anyone can search for and see this list. You can also share using a link.', 'wc_wishlist' ); ?></span></label>
					</td>
				</tr>
				<tr>
					<td><input type="radio" name="wishlist_sharing" id="rad_shared" value="Shared" <?php checked( 'Shared', $sharing ); ?>><label for="rad_shared"><?php _e( 'Shared', 'wc_wishlist' ); ?> <span class="wl-small">- <?php _e( 'Only people with the link can see this list. It will not appear in public search results.', 'wc_wishlist' ); ?></span></label></td>
				</tr>
				<tr>
					<td><input type="radio" name="wishlist_sharing" id="rad_priv" value="Private" <?php checked( 'Private', $sharing ); ?>><label for="rad_priv"><?php _e( 'Private', 'wc_wishlist' ); ?> <span class="wl-small">- <?php _e( 'Only you can see this list.', 'wc_wishlist' ); ?></span></label></td>
				</tr>
			</table>
		</p>  
		</div>
	...
	public function nameinfo_metabox( $post ) {
		...
		<div class="wl-admin-wrapper">
			...
			<?php endif; ?>
			<p class="form-row form-row-full">
				<label for="wishlist_owner"><?php _e( 'User ID Associated with the List', 'wc_wishlist' ); ?></label>
				<input type="text" name="wishlist_owner" id="wishlist_owner" value="<?php echo esc_attr( $wishlist_owner ); ?>" />
			</p>
			...
			<p class="form-row">
				...
				<table class="wl-rad-table">
					<tr>
						...
						<td><label for="rad_notification_yes"><?php _e( 'Yes', 'wc_wishlist' ); ?> <span class="wl-small">- <?php _e( 'Send an email if a price reduction occurs.', 'wc_wishlist' ); ?></span></label></td>
					</tr>
					<tr>
						<td><input type="radio" id="rad_notification_no" name="wishlist_owner_notifications" value="no" <?php checked( 'no', $notifications ); ?>></td>
						<td><label for="rad_notification_no"><?php _e( 'No', 'wc_wishlist' ); ?> <span class="wl-small">- <?php _e( 'Do not send an email if a price reduction occurs.', 'wc_wishlist' ); ?></span></label></td>
					</tr>
				</table>
			</p>  
		</div>
							...
							if ( $_product->exists() && $item['quantity'] > 0 ) {
								...
								<tr class="item <?php if ( !empty( $class ) ) echo $class; ?>" data-order_item_id="<?php echo $item_id; ?>">
									...
									<td class="name" style="text-align:left;">
										...
										if ( WC_Wishlist_Compatibility::is_wc_version_gte_2_1() ) :
											if ( isset( $item['variation'] ) ) :
												echo '<br />' . wc_get_formatted_variation( $item['variation'], true );
											endif;
										else :
									...
									<td class="quantity" width="1%">
										...
										<input type="number" step="<?php echo apply_filters( 'woocommerce_quantity_input_step', '1', $_product ); ?>" min="0" autocomplete="off" name="wishlist_item_quantity[<?php echo $item_id; ?>]" placeholder="0" value="<?php echo esc_attr( $product_quantity_value ); ?>" size="4" class="quantity" />
									</td>
									<!-- Product price -->
									<td class="line_cost" width="1%">
										<?php
										if ( WC_Wishlist_Compatibility::is_wc_version_gte_2_1() ) {
											$price = apply_filters( 'woocommerce_cart_item_price', $cart->get_product_price( $_product ), $item, $item_id );
										} else {
											...
											$price = apply_filters( 'woocommerce_cart_item_price_html', woocommerce_price( $product_price ), $item, $item_id );
										}
										?>
